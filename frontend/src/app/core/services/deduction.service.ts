...
export class DeductionService {
  ...
  private readonly COL = "deductions";

  getDeductions(): Observable<DeductionRecord[]> {
    return this.http.get<any>(`${BASE}/${this.COL}?key=${API_KEY}`).pipe(
      map(res => {
        const docs: DeductionRecord[] = (res.documents || []).map(fromDoc);
        return docs.sort((a, b) => String(b.deductionDate).localeCompare(String(a.deductionDate)));
      }),
      catchError(() => of([]))
    );
  }
  ...
  addDeduction(payload: Omit<DeductionRecord, "id" | "isRecovered">): Observable<DeductionRecord> {
    const body = { fields: toFields({ ...payload, isRecovered: 0 }) };
    return this.http.post<any>(`${BASE}/${this.COL}?key=${API_KEY}`, body).pipe(
      map(doc => fromDoc(doc) as DeductionRecord)
    );
  }
}
