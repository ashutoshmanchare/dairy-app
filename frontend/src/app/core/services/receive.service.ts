...
export class ReceiveService {
  ...
  private readonly COL = "milk_received";

  getReceives(): Observable<ReceiveRecord[]> {
    return this.http.get<any>(`${BASE}/${this.COL}?key=${API_KEY}`).pipe(
      map(res => {
        const docs: ReceiveRecord[] = (res.documents || []).map(fromDoc);
        return docs.sort((a, b) => String(b.receivedDate).localeCompare(String(a.receivedDate)));
      }),
      catchError(() => of([]))
    );
  }
  ...
  createReceive(payload: ReceiveRecord): Observable<ReceiveRecord> {
    ...
    return this.http.post<any>(`${BASE}/${this.COL}?key=${API_KEY}`, body).pipe(
      map(doc => fromDoc(doc) as ReceiveRecord)
    );
  }
