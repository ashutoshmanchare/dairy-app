...
export class MilkSaleService {
  ...
  createMilkSale(payload: MilkSaleRecord): Observable<MilkSaleRecord> {
    ...
    const body = { fields: toFields({ ...payload, totalAmount }) };
    return this.http.post<any>(`${BASE}/${this.COL}?key=${API_KEY}`, body).pipe(
      map(doc => fromDoc(doc) as MilkSaleRecord)
    );
  }
